...
import com.blackgear.platform.core.networking.payloads.ClientboundEntityDataPayload.Entry;
...
public class SyncedEntityDataContainer {
    ...
    public void flush() {
        ...
        if (!tracking.isEmpty()) {
            PayloadDistributor.sendToPlayersTrackingEntity(this.entity, new ClientboundEntityDataPayload(this.entity.getId(), tracking));
        }
        
        if (!self.isEmpty() && this.entity instanceof ServerPlayer player) {
            PayloadDistributor.sendToPlayer(player, new ClientboundEntityDataPayload(this.entity.getId(), self));
        }
    ...
    public void syncTrackingDataTo(ServerPlayer player) {
        ...
        if (!entries.isEmpty()) PayloadDistributor.sendToPlayer(player, new ClientboundEntityDataPayload(this.entity.getId(), entries));
    }
