...
public record SyncedDataKey<T>(ResourceLocation id, Codec<T> codec, Supplier<T> fallback, SyncMode mode) {
    public static final Map<ResourceLocation, SyncedDataKey<?>> KEYS = new ConcurrentHashMap<>();
    
    public static <T> SyncedDataKey<T> create(ResourceLocation id, Codec<T> codec, Supplier<T> fallback, SyncMode mode) {
